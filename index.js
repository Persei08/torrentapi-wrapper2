...
var got = require('got'),
    ...
    appid = Math.random().toString(36).substr(2, 6);
...
var getToken = function () {
    return new Promise(function (resolve, reject) {
        if (session.token && (session.token_expires - Date.now() > 0)) {
            ...
            custom(endpoint + '?get_token=get_token&app_id=' + appid).then(function (res) {
...
var buildUrl = function (q) {
    var url = endpoint + '?app_id=' + appid + '&limit=100&', count = 1;
...
var chainCall = function (ua, query, mode) {
    ...
    }).then(buildUrl).then(custom).then(parseResponse);
};
